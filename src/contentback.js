...
//return htmlToImage
//  .toPng(node, {skipFonts: true, filter : filter})
//  .then((dataUrl) => {
//    const img = new Image();
//    img.src = dataUrl;
//    return dataUrl;
...
//	const ballB = Bodies.circle(460, 10, 40, 10);
	const ground = Bodies.rectangle(window.innerWidth/2, window.innerHeight + 25, window.innerWidth+200, 60, { isStatic: true });
	Composite.add(engine.world, [ground]);
	try{
	for (let i = 0; i < DOMS.length; i++){
		
		let domimg = await domtoimg(DOMS[i]);
		let dscale = 1;
//		if (DOMS[i].tagName === 'IMG' && DOMS[i].hasAttribute('src')){
//			console.log(DOMS[i]);
//			domimg = DOMS[i].getAttribute('src');
//			console.log(DOMS[i].getAttribute('height') + "x" + DOMS[i].getAttribute('width'));
//			console.log(DOMS[i].getBoundingClientRect().height + " x " + DOMS[i].getBoundingClientRect().width)
//			dscale = DOMS[i].getBoundingClientRect().height * 0.60;
//		}
		//document.body.append(domimg);
		let bounds = DOMS[i].getBoundingClientRect();
		if (!(domimg === 'undefined' || domimg === 'data:,')){
			...
			Composite.add(engine.world, [dombox]);
		}
	}
	}
	catch(DOMException){
...
function levelselector(){
	...
	document.addEventListener("keydown", (event) =>{
		if(event.key === 'Escape' && mttrrunning === 1 && document.querySelector("#mttrcvs")){
				document.querySelector("#mttrcvs").remove();
				console.log(unsdom);
				for (let i = 0; i < unsdom.length; i++){
					unsdom[i].style.opacity = "";
				}
			mttrrunning = 0;
		}
	})
